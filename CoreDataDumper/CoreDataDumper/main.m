...
NSManagedObjectContext *managedObjectContext(NSString *filePath) {
    ...
    NSLog(@"url = %@", url);
    NSPersistentStore *newStore = [coordinator persistentStoreForURL:url];
    if (newStore == nil) {
...
{
    if (argc < 2) {
        NSLog(@"Usage: CoreDataDumper <path_to_persitent_store>");
        exit(255);
    }
    @autoreleasepool {
        ...
        NSManagedObjectContext *moc = managedObjectContext([NSString stringWithUTF8String:argv[1]]);
